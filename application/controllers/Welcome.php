...
class Welcome extends CI_Controller
{
	...
	public function addNewBarcodeData()
	{
		...
		$barcodecheck = $this->generic->GetData('ipr_product_detail', array('barcodeNo' => $this->input->post('barcodeNumber')));
		if ($barcodecheck) {
			$this->session->set_flashdata('barcodeaddedalready', 1);
			redirect(base_url('products-detail/?order_id=' . $this->uri->segment(2)));
		} else {
			...
			$this->session->set_flashdata('barcodeadded', 1);
			redirect(base_url('products-detail/?order_id=' . $this->uri->segment(2)));
		}
	}
...
public function deletIPRProductDetail(){
	$productdetail=$this->generic->GetData('ipr_product_detail',array('productID'=>$_GET['product_id']));
	$this->generic->delet('ipr_product_detail',array('productID'=>$_GET['product_id']));
	$this->session->set_flashdata('productdeletd', 1);
		redirect(base_url('products-detail/?order_id=' . $productdetail[0]['orderID']));
}
